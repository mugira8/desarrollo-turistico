...
function loadPlaces(){
    ...
    lugares.forEach(element => {
        console.log(element)
        document.getElementById('places').innerHTML += "<div class='col-6 col-md-6 col-lg-4 g-3 '><div class='card background h-100 card-body p-5'><h5>"+element+"</h5><button class='btn btn-primary type='button' data-bs-toggle='offcanvas' data-bs-target='#exampleModal' btn-rounded btn-sm'>"+Math.floor(Math.random() * 200)+" Tours y actividades</button></div>"
    });
